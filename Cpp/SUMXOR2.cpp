...
inline base operator+(base a, base b) { return {a.x + b.x, a.y + b.y}; }

inline base operator-(base a, base b) { return {a.x - b.x, a.y - b.y}; }

inline base operator*(base a, base b) { return {a.x * b.x - a.y * b.y, a.x * b.y + a.y * b.x}; }

inline base conj(base a) { return {a.x, -a.y}; }

int lim = 1;
vector<base> roots = {{0, 0},
                      {1, 0}};
vector<int> rev = {0, 1};
...
struct poly {
    ...
    bool is_zero() const {
        for (int i = 0; i < size(); i++) if (a[i] != 0) return false;
        return true;
    }
    ...
    poly operator*(const poly &b) const {
        ...
        for (auto x: res) ans.emplace_back(x);
        while ((int) ans.size() && ans.back() == 0) ans.pop_back();
        return ans;
    }
    ...
    poly reverse_it(int n, bool rev = false) const {
        poly ans(*this);
        if (rev) { ans.a.resize(max(n, (int) ans.a.size())); }
    ...
    poly inverse(int n) const {
        ...
        assert(a[0] != 0);
        poly ans{mint(1) / a[0]};
        for (int i = 1; i < n; i *= 2) { ans = (ans * mint(2) - ans * ans * mod_xk(2 * i)).mod_xk(2 * i); }
        return ans.mod_xk(n);
    }
    ...
    pair<poly, poly> divmod(const poly &b) const {
        ...
        poly D = (reverse_it(d + 1) * b.reverse_it(d + 1).inverse(d + 1)).mod_xk(d + 1).reverse_it(d + 1, true);
        return {D, *this - (D * b)};
    }
